...
void DisplayFunctionGGBA::init( float gamma, float L_max, float L_black, float E_amb, float screen_refl )
{
  this->gamma = gamma;
  this->L_max = L_max;
...
void DisplayFunctionGGBA::print( FILE *fh )
{
  ...
  fprintf( fh, "   gamma = %g\t L_max = %g\t L_black = %g\n", (double)gamma, (double)L_max, (double)L_black );
  fprintf( fh, "   E_amb = %g\t k     = %g\n", (double)E_amb, (double)screen_refl );
}
...
DisplayFunctionLUT::DisplayFunctionLUT( const char *file_name ) : pix_lut( NULL ), L_lut( NULL )
{
  ...
  if( fh == NULL )
    throw pfs::Exception( "Cannot open lookup-table file" );
...
inline float bin_search_interp( float x, const float *lut_x, const float *lut_y, const int lutSize )
{
  if( x <= lut_x[0] )
    return lut_y[0];
...
void DisplayFunctionLUT::print( FILE *fh )
{
  ...
  fprintf( fh, "   L_min = %g \tL_max = %g\n", (double)pow( 10, L_lut[0] ), (double)pow( 10, L_lut[lut_size-1] ) );
}
